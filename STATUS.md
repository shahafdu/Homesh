...
| ⏳ | **Backups that leave the house** | Encrypted here, with a key that stays here, and pushed to a Drive folder after each daily one. The direction is the whole security argument: **the house pushes and nothing out there pulls** -- there is no server of ours online to break into, only a folder holding ciphertext and a filename, and no route back to anything. Encryption is not decoration on this one: a Homesh backup is the catalog, and the catalog names the rooms in the house, the screens in them and the folders on the disks. In the clear, on somebody else's disk, that is a map. **Two things are needed from you** -- a Drive folder shared as Editor, and `tools
