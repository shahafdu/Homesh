...
| ⏳ | **Backups that leave the house** | Encrypted, scheduled and restorable -- all of it verified on the real database, 569,453 rows sealed and brought back byte-identical. Only the destination is left. **Drive cannot be it**: a service account has no storage quota, so it cannot create a file even in a folder shared with it as Editor, and Google's two remedies are both Workspace features. Settled on **Oracle Object Storage** -- free, S3-compatible, and the same account hosts the always-on node later, which was the reason you asked. **[docs/OFFSITE_BACKUPS.md](docs/OFFSITE_BACKUPS.md) is what to do**; six steps, and the last one is pasting two keys into `.env` |
