...
Last updated: 14 September 2026 · 508 tests · 23 migrations · CI green
...
| ⏳ | **Backups that leave the house** | Built, encrypted, and **blocked by Google.** The encryption and the round trip work on the real database -- 569,453 rows, 18.7 MB, sealed and brought back byte-identical. The destination does not: a service account has **no storage quota of its own**, so it cannot create a file in Drive even inside a folder shared with it as Editor. Google's two remedies, shared drives and domain-wide delegation, are both Workspace features. Reading your folders is unaffected and always was. **Needs a decision from you** on where the copies should go instead -- object storage with a write-only key is the better answer anyway, since then even this machine could not read or delete them |
