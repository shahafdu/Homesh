...
Last updated: 22 September 2026 · 692 tests · 26 migrations · CI green
...
| 🟡 | **An mp4 that started four seconds in** | Fixed on the server, so every screen has it after a restart of the app -- no install. Particular mp4s began four seconds in on a television, always exactly four, immediately, and went back there when taken to the beginning from the tower. The file was never at fault: its sync table starts at the first sample, it carries no edit list, and the same file plays from the beginning on the phone. What changed was which player got it -- fixing the bridge to the box's own decoder sent every direct-play video to Android's MediaPlayer, and that is the one that skips. Video goes to the web view again, which is what played these files correctly all along; the box keeps Matroska and transport streams, which Chromium cannot open at all, and anything Chromium turns out to refuse |
| 🟡 | **TV app 0.6.3 on the boxes** | In daily use. Each screen offers the update itself on launch; Android's own installer prompt cannot be skipped, so it needs a press per box. 0.6.3 made the box's player always seek to the position it was given; 0.6.2 fixed the crash on video and on stop, the film down one edge, and put the on-screen controls over the film. Each room card shows the version its box reported |
