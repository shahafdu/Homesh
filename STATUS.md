...
Last updated: 15 September 2026 · 585 tests · 24 migrations · CI green
...
| | What | Why it matters |
|---|---|---|
| ⏳ | **Create the Oracle standby machine** | Ampere A1 at whatever size the console marks Always Free-eligible (1 OCPU / 6 GB in Jerusalem, and never raise it), Ubuntu 24.04 aarch64, the PC's SSH key pasted in -- exact clicks in `docs/STANDBY.md`. Jerusalem often has no free Ampere capacity; if it says *Out of host capacity*, try again later, and never via a paid A2 machine. Put its public IP in `.local\standby-ip` |
...
| 🟡 | **Backups, and putting one back** | Settings → Backups, administrators only. Hourly for a day, one a day for a week, one a week for five weeks -- so there is always a point about a fortnight and about a month back. Restoring takes a copy of the present state first, so restoring the wrong one is undoable. The media is not backed up and should not be -- this is everything the server knows *about* your files, which is the part that exists nowhere else. A backup can be downloaded, and should be: one that lives on the same disk as the thing it protects is half a backup |
