...
| 🔴 | **Install TV app 0.6.3 on the boxes** | Each screen offers the update itself on launch; Android's own installer prompt cannot be skipped, so it needs a press per box. **0.6.3 fixes a video starting where the previous one was left** -- Android's VideoView keeps a seek position of its own and applied it to whatever was opened next, which is why some mp4s began four seconds in and why taking one back to the beginning from the tower jumped straight back. 0.6.2 before it fixed the crash on video and on stop, the film down one edge, and put the on-screen controls over the film. Each room card shows the version its box reported |
