    ...
    return JSONResponse(
        {
            ...
            # from home needs answered.
            "origin": get_settings().public_origin.rstrip("/") or None,
...
async def phone_apk() -> Response:
    ...
    Not the interface — the browser is, and that is where the passkey works.
    This exists for the one thing a web page cannot do for itself: when Android
    has stopped Tailscale and the server cannot be reached, open Tailscale.
    """
    apk = _tv_apk_path().with_name("homesh-phone.apk")
    if not apk.is_file():
        return JSONResponse(
            {"detail": "No phone app build present. Run APP=phone tools/build-tv-apk.sh."},
            status_code=404,
        )

    version = "unknown"
    info = apk.with_name("homesh-phone.json")
    if info.is_file():
    ...
    return FileResponse(
        ...
        filename=f"homesh-phone-{version}.apk",
